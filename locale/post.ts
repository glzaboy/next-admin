const i18n = {
  "en-US": {
    "post.category.title": "Categories Manager",
    "post.new": "New",
    "post.edit.Title": "Edit Category",
    "post.category.edit": "Edit",
    "post.category.field.name": "Category",
    "post.category.field.createTime": "Create Time",
    "post.category.field.updateTime": "Update Time",
    "post.category.delete": "Delete",
  },
  "zh-CN": {
    "post.category.title": "类别管理",
    "post.new": "新建",
    "post.edit.Title": "编辑类别",
    "post.category.edit": "编辑",
    "post.category.field.name": "类别",
    "post.category.field.createTime": "创建时间",
    "post.category.field.updateTime": "更新时间",
    "post.category.delete": "删除",
  },
};

export default i18n;
